...
function EPGP:CanResetEPGP()
  return C_GuildInfo.CanEditOfficerNote() and GS:IsCurrentState()
end
...
function EPGP:CanDecayEPGP()
  if not C_GuildInfo.CanEditOfficerNote() or self.db.profile.decay_p == 0 or not GS:IsCurrentState() then
    return false
  end
  return true
end
...
function EPGP:CanIncEPBy(reason, amount)
  if not C_GuildInfo.CanEditOfficerNote() or not GS:IsCurrentState() then
    return false
  end
...
function EPGP:PrintCoinLog(num, show_gold)
  ...
  for _, e in pairs(to_show) do
    ...
    if e.currencyID ~= nil and tonumber(e.currencyID) > 0 then
      local currency_name = C_CurrencyInfo.GetCurrencyInfo(e.currencyID).name
