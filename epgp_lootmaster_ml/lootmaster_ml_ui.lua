...
function LootMasterML:GetFrame()
    ...
    local frame = CreateFrame("Frame","LootMasterMLFrame",mainframe, BackdropTemplateMixin and "BackdropTemplate")
    ...
    local extralootframe = CreateFrame("Frame","LootMasterMLFrameExtraLoot",frame, BackdropTemplateMixin and "BackdropTemplate")
    ...
    local titleFrame = CreateFrame("Frame", nil, mainframe, BackdropTemplateMixin and "BackdropTemplate")
    ...
    local equipHeaderFrame = CreateFrame("Frame", nil, frame, BackdropTemplateMixin and "BackdropTemplate")
    ...
    local tbGPValueFrame = CreateFrame("Frame", nil, frame, BackdropTemplateMixin and "BackdropTemplate")
            ...
            if not loot.manual then
				...
				if loot.allowBids then
					...
					if bid>loot.gpvalue then
						info.isTitle = false;
						info.disabled = not C_GuildInfo.CanEditOfficerNote();
...
function LootMasterML:ShowRaidInfoLookup()
    ...
    if not frame then
        ...
        frame = CreateFrame("Frame","LootMasterUIFrame",UIParent, BackdropTemplateMixin and "BackdropTemplate")
        --#region Setup main masterlooter frame
        frame:Hide();
        frame:SetWidth(700)
        frame:SetHeight(420)
        ...
        local titleFrame = CreateFrame("Frame", nil, frame, BackdropTemplateMixin and "BackdropTemplate")
        --#region Setup main frame title
        titleFrame:SetBackdrop({
            bgFile = "Interface\\ChatFrame\\ChatFrameBackground",
            edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
		...
		local tbWhisperFrame = CreateFrame("Frame", nil, frame, BackdropTemplateMixin and "BackdropTemplate")
